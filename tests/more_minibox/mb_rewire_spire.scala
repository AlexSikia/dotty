...
class Cplx[@specialized A](real: A, imag: A)(implicit f: Fractional[A]) {
  def isZero: Boolean = ???
  def abs: A = ???
...
trait CplxIsRing[@specialized A] {
  implicit def f: Fractional[A]
  def one: Cplx[A] = Cplx.one
}

trait Ord[@specialized A] {
  def lt(o1: A, o2: A): Boolean
}

trait Fractional[@specialized A] extends Ord[A] {
  def one: A
  def zero: A
}


trait CplxIsEuclideanRing[@specialized A] extends CplxIsRing[A] {
